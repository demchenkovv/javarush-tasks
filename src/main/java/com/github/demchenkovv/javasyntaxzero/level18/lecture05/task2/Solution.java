package com.github.demchenkovv.javasyntaxzero.level18.lecture05.task2;
...
public class Solution {
    ...
    public static boolean hasExpiredProduct(Stream<Product> productStream) {
        return productStream
                .anyMatch(date -> date.getExpirationDate().isBefore(LocalDate.now()));

//        LocalDate now = LocalDate.now();
//        return productStream
//                .map(Product::getExpirationDate)
//                .anyMatch(date -> date.isBefore(now));
    }
}
...
/*
В классе Solution метод hasExpiredProduct(Stream<Product>) возвращает true, если во входящем
потоке товаров есть хотя бы один просроченный и возвращать false, если таких товаров нет.

Чтобы это проверить, нужно у каждого товара в потоке сравнить поле expirationDate (годен до)
типа LocalDate с текущей датой (статический метод now() класса LocalDate). Если срок годности
меньше (строго) текущей даты, то в этом потоке есть просроченные товары.
