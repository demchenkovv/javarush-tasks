package com.github.demchenkovv.javasyntaxzero.level09.lecture01.task3;
...
public class Solution {
    // индекс строки соответствует значению 0x... (16) системы счисления
    ...
    public static String toHex(int decimalNumber) {
        ...
        while (decimalNumber != 0) {
            char currentChar = HEX.charAt(decimalNumber % 16);
    ...
    public static int toDecimal(String hexNumber) {
        if (hexNumber == null || hexNumber.isBlank()) return 0;

        int result = 0;
        for (int i = 0; i < hexNumber.length(); i++) {
            char currentChar = hexNumber.charAt(i); // текущий символ hexNumber
            int charIndexFromHEX = HEX.indexOf(currentChar); // индекс HEX, равный текущему символу hexNumber
            result = 16 * result + charIndexFromHEX;
        }
        return result;
    }
}
