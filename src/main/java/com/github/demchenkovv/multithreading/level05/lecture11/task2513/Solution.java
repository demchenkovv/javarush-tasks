package com.github.demchenkovv.multithreading.level05.lecture11.task2513;
...
public class Solution {
    ...
    /**
     * Thread.yield() - это РЕКОМЕНДАЦИЯ планировщику, что текущий поток готов уступить свое
     * текущее использование процессора. Планировщик может игнорировать эту подсказку.
     * Таким образом, нет гарантий, что текущий поток, на котором вызвали yield(), сразу же не займет место.
     */
    public static synchronized void moveMoney(Account from, Account to, int amount) {
        ...
        Thread.yield(); // дать возможность проверить баланс (списание) второму потоку (controlThread)
        ...
        Thread.yield(); // дать возможность проверить баланс (зачисление) второму потоку (controlThread)
    }
