package com.github.demchenkovv.javacore.level06.lecture05.task2;
...
public class Solution {
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException {
        ...
        for (Horse horse : horses) {
            if (!horse.isFinished()) {      // Пока метод run() в классе Horse не завершит свою работу, isFinished == false
                System.out.println("Waiting for " + horse.getName());   // имя текущего потока
                horse.join();       // ожидание, когда текущая лошадь завершит метод run()
            } else
                finishedCount++;    // если метод run() у текущей лошади завершился - считаем её финишировавшей
        }
