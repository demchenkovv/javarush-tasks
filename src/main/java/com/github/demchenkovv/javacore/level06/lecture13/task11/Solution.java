package com.github.demchenkovv.javacore.level06.lecture13.task11;
...
public class Solution {
    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            ...
            // проснувшиеся потоки зайдя в синхронизированный блок кода не будут зависать на вводе //2,
            // поскольку условие if (reader.ready()) должно будет возвращать false // 1.
            while (!isInterrupted()) {
                try {
                synchronized (reader) {
                    if (reader.ready()) { // 1
            ...
            // Вариант 2 (может не сработать) . Объяснения см. скриншоты в текущей папке или по ссылке
...
//                try {
//
//                    // 1) проверяет, можно ли прочесть что-то из буффера ввода, и если там что-то есть,
//                    // запускается блокирующий метод 2)
//                    if (reader.ready()) {
//
//                        // 2) все три потока могут зависнуть на данном блокирующем методе
//                        // Как только пользователь нажмет энтер, первый проснувшийся поток сможет прочитать
//                        // строку и сохранит её себе. Заметьте, что оставшиеся два потока так и будут висеть на //2) - и это неправильно
